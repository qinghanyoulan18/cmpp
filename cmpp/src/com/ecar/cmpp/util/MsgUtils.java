...
import com.ecar.cmpp.entity.MsgCancel;
import com.ecar.cmpp.entity.MsgCancelResp;
...
import com.ecar.cmpp.entity.MsgQuery;
import com.ecar.cmpp.entity.MsgQueryResp;
import com.ecar.cmpp.entity.MsgSubmit;
import com.ecar.cmpp.entity.MsgSubmitResp;
import com.ecar.cmpp.spClient.Client;
...
public class MsgUtils {
	 ...
	 * @return
	 */
	public static byte[] packMsg(MsgHead msg) throws Exception{
		//生成一段内存字节流
		...
		switch (command) {
			// 接收到的为.CMPP_CONNECT消息类型时 处理如下
		        case MsgCommand.CMPP_CONNECT:
					...
					MsgConnect connect = (MsgConnect) msg;
					...
					sb.append("authenticatorSource:"+connect.getAuthenticatorSource());
		        ...
		        case MsgCommand.CMPP_TERMINATE:
					...
					dos.write(len);
					byte[] cmd = Util.writeInt(MsgCommand.CMPP_TERMINATE);
					dos.write(cmd);
		        ...
		        case MsgCommand.CMPP_TERMINATE_RESP:
					...
					dos.write(len1);
					byte[] cmd1 = Util.writeInt(MsgCommand.CMPP_TERMINATE_RESP);
					dos.write(cmd1);
		        ...
		        case MsgCommand.CMPP_SUBMIT:
		        	MsgSubmit submit = (MsgSubmit)msg;
		        	//-------------------写入长度
...
//					System.out.println("getMsg_level()："+submit.getMsg_level());
		        ...
		        case MsgCommand.CMPP_DELIVER_RESP:
					MsgDeliverResp deliver_resp = (MsgDeliverResp) msg;
		        ...
		        case MsgCommand.CMPP_CANCEL:
		        	MsgCancel cancel = (MsgCancel) msg;
		        ...
		        case MsgCommand.CMPP_ACTIVE_TEST:
		        	MsgActiveTest act = (MsgActiveTest)msg;
		        	byte[] len6 = Util.writeInt(act.getMsg_length());
					dos.write(len6);
					byte[] cmd6 = Util.writeInt(MsgCommand.CMPP_ACTIVE_TEST);
					dos.write(cmd6);
		        ...
		        case MsgCommand.CMPP_ACTIVE_TEST_RESP:
		        	MsgActiveTestResp act_resp = (MsgActiveTestResp)msg;
		        	byte[] len7 = Util.writeInt(act_resp.getMsg_length());
					dos.write(len7);
					byte[] cmd7 = Util.writeInt(MsgCommand.CMPP_ACTIVE_TEST_RESP);
					dos.write(cmd7);
	 ...
	 * @return 对应的消息头
	 */
	public static MsgHead praseMsg(byte[] msg) throws Exception{
		//将字节流封装成内存流
		...
		DataInputStream dis = new DataInputStream(bais);
			...
			// 当接收到得消息类型为CMPP_CONNECT_RESP 时
				case MsgCommand.CMPP_CONNECT_RESP:
					MsgConnectResp msg_resp = new MsgConnectResp();
				...
				case MsgCommand.CMPP_SUBMIT_RESP:// 要解包Submit应答消息
					...
					return msr;
				case MsgCommand.CMPP_QUERY_RESP://查询消息应答
					MsgQueryResp query = new MsgQueryResp();
				...
				case MsgCommand.CMPP_DELIVER:// 如果是deliver消息
					...
					msd.setMsg_Length((byte) (msg.length + 4));// 总长
				...
				//删除消息
				case MsgCommand.CMPP_CANCEL: 
					MsgCancelResp can_resp = new MsgCancelResp();
				...
				case MsgCommand.CMPP_ACTIVE_TEST:// 链路检测
					MsgActiveTest act = new MsgActiveTest();
					act.setMsg_length(12);
					act.setMsg_command(command_Id);
					act.setMsg_squence(sequenceId);
					return act;
				case MsgCommand.CMPP_ACTIVE_TEST_RESP:// 链路检测应答  13字节
					MsgActiveTestResp act_resp = new MsgActiveTestResp();
